...
#     name: python3
###############
# py .\extract_tiles_from_wsi_openslide.py --bgremoved  -l 0 -p 2000 C:/research/QuPath/Movat/C*V-DC-20.svs -o .\imgs\
###############
# ---
    ...
    for y in tqdm(range(0,osh.level_dimensions[0][1],round(patch_size * osh.level_downsamples[level])), desc="outer" , leave=False):
        for x in tqdm(range(0,osh.level_dimensions[0][0],round(patch_size * osh.level_downsamples[level])), desc=f"innter {y}", leave=False):
            ...
            if(args.bgremoved):
                ...
                #print(f"{fnamebase}_{level}_{y}_{x} = {gpatchmean}")
                #if(gpatchmean>240 or gpatchmean<50):
                if(gpatchmean>235 or gpatchmean<50):
                    #misses.append(f"{fnamebase}_{level}_{y}_{x} = {gpatchmean}")
                    continue
                #else:
                    #print(f"{fnamebase}_{level}_{y}_{x} = {gpatchmean}")
                    #hits.append(f"{fnamebase}_{level}_{y}_{x} = {gpatchmean}")

        
            patch=cv2.cvtColor(patch,cv2.COLOR_RGB2BGR)
            cv2.imwrite(outdir + f"{fnamebase}_{level}_{y}_{x}.png",patch)
    osh.close()
    #print(f"{fnamebase} Hits = {len(hits)}, Misses = {len(misses)}")
